 ...
 * @author Ivaa
 */
public class LaporDitemukan extends AppFrame {
    ...
    public LaporDitemukan() { this(null); }
 
    public LaporDitemukan(JFrame parentFrame) {
        super("Lapor Barang Ditemukan", AppTheme.WINDOW_FORM, parentFrame);
        ...
        topBar.add(AppLabelFactory.sectionTitle("Lapor Barang Ditemukan"), BorderLayout.WEST);
        ...
        JLabel lblBanner = new JLabel(
            "<html><b>Barang Ditemukan</b> — Isi data barang yang Anda temukan dan belum tahu pemiliknya. "
            + "Pemilik asli dapat mengajukan klaim untuk mengambil barangnya.</html>");
        lblBanner.setFont(new Font("Segoe UI", Font.PLAIN, 11));
        lblBanner.setForeground(new Color(6, 95, 70));
        ...
        addFormRow(formCard, "Lokasi Ditemukan *", lx, y);
        txtLokasi = styledField();
        txtLokasi.setToolTipText("Di mana Anda menemukan barang ini, mis: Kantin Fakultas");
        txtLokasi.setBounds(fx, y, fw, rowH);
        formCard.add(txtLokasi); y += gap;
 
        // Status badge (read-only)
        addFormRow(formCard, "Status Laporan", lx, y);
        JLabel lblStatus = new JLabel("  DITEMUKAN  ") {
            @Override
            protected void paintComponent(Graphics g) {
                Graphics2D g2 = (Graphics2D) g.create();
                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
        ...
        lblStatus.setBounds(fx, y + 5, 110, 26);
    ...
    private void simpanData() {
        ...
        if (lokasi.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Lokasi ditemukan wajib diisi!", "Validasi", JOptionPane.WARNING_MESSAGE);
        ...
        barang.setStatus("Ditemukan");         // hardcoded
        ...
        JOptionPane.showMessageDialog(this,
            ...
            + "Pemilik asli barang dapat mengajukan klaim untuk mengambilnya.");
        resetForm();
    }
